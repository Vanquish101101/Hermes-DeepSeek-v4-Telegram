...
## version 0.3.3 | 3d-web-experience: зависимость доустановлена и протестирована | 2026-06-17

**Изменения:**
- Установлен `@gltf-transform/cli` v4.4.0 глобально (npm) — закрывает зависимость скилла `3d-web-experience` (см. v0.3.2)
- Скилл протестирован на синтетической тестовой модели (треугольник, сгенерирован через `@gltf-transform/core`): `inspect` → `optimize --compress draco` → `validate` — весь пайплайн прошёл без ошибок и предупреждений
- TODO по зависимостям скиллов обновлён — пункт `3d-web-experience` закрыт
...
**Изменения:**
- Получен API-ключ YouGile через 2 REST-запроса (companies → keys), в интерфейсе YouGile кнопки генерации ключа нет
- Подключён `@nebelov/yougile-mcp` (npx) — пакет обновился с момента первой проверки: теперь 57 инструментов и полное покрытие API v2 (раньше было ~30%)
- Инструменты: проекты, доски, колонки, задачи, чаты/сообщения, пользователи, департаменты, роли, стикеры, вебхуки
- Gateway перезапущен, всего подключено 23 MCP сервера
- В ожидании: Google Calendar (OAuth на стороне MCP Market) и Redis (нужен host:port от пользователя)
