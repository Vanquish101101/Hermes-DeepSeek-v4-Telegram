...
## ⚠️ Внимание (добавлено в версии 1.2.0)
Helicone закрывает учёт расходов на **DeepSeek** (основная LLM). Для **Perplexity** свой, более простой
механизм — API сам возвращает точную стоимость в каждом ответе, без прокси. Подробности и настройка
Perplexity MCP — отдельный файл **`Perplexity-setup.md`** (документирован отдельно, т.к. был подключён
раньше Helicone, но без описания в снимке версий — этот пробел закрыт версией 1.2.0).
